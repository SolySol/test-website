...
if (isset($_POST['submit'])) {
    ...
    $select_admins = mysqli_prepare($conn, "SELECT * FROM `admins` WHERE name = ?");
    mysqli_stmt_bind_param($select_admins, 's', $name);
    mysqli_stmt_execute($select_admins);
    mysqli_stmt_store_result($select_admins);

    if (mysqli_stmt_num_rows($select_admins) > 0) {
        $warning_msg[] = 'Username already taken!';
    } else {
        if ($pass != $c_pass) {
            $warning_msg[] = 'Password not matched!';
            ...
            // Insert new admin
            $insert_admin = mysqli_prepare($conn, "INSERT INTO `admins` (name, password) VALUES (?, ?)");
            mysqli_stmt_bind_param($insert_admin, 'ss', $name, $c_pass);
