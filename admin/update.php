...
// Fetch profile data
$select_profile = mysqli_prepare($conn, "SELECT * FROM `admins` WHERE id = ? LIMIT 1");
mysqli_stmt_bind_param($select_profile, 's', $admin_id);
mysqli_stmt_execute($select_profile);
$result = mysqli_stmt_get_result($select_profile);
...
if (isset($_POST['submit'])) {
   ...
   if (!empty($name)) {
       $verify_name = mysqli_prepare($conn, "SELECT * FROM `admins` WHERE name = ?");
       mysqli_stmt_bind_param($verify_name, 's', $name);
       mysqli_stmt_execute($verify_name);
       mysqli_stmt_store_result($verify_name);
       if (mysqli_stmt_num_rows($verify_name) > 0) {
           $warning_msg[] = 'Username already taken!';
       } else {
           $update_name = mysqli_prepare($conn, "UPDATE `admins` SET name = ? WHERE id = ?");
           mysqli_stmt_bind_param($update_name, 'ss', $name, $admin_id);
   ...
   if ($old_pass != $empty_pass) {
       if ($old_pass != $prev_pass) {
           ...
           if ($new_pass != $empty_pass) {
               $update_password = mysqli_prepare($conn, "UPDATE `admins` SET password = ? WHERE id = ?");
               mysqli_stmt_bind_param($update_password, 'ss', $c_pass, $admin_id);
